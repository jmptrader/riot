...
function mkdom(template) {
  ...
  if (riot.hack.mkdom) riot.hack.mkdom(el, template, tagName)
  else el.innerHTML = template
...
// minimize risk: only 0 or 1 _space_ between attr & _quoted_ value
function walkAttributes(html, fn) {
  var m,
      re = /([-\w]+) ?= ?(?:"([^">]+)|'([^'>]+))/g

  while ((m = re.exec(html))) {
    fn(m[1].toLowerCase(), m[2] || m[3])
  }
}
  ...
  each(dom.attributes, function(attr) {
    ...
    if (attr.name === 'id' || attr.name === 'name') {
      ...
      if (!p)
        ...
        Array.isArray(p) ? p.push(dom) : (parent[v] = [p, dom])
    }
  })
}

// eof browser/tag/util.js
